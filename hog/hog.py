...
def free_bacon(score):
    ...
    while score > 0:
        if pre == -1:
            pre, score = score % 10, score // 10
        else:
            ans, score, pre = ans + pre - score % 10, score // 10, -1
    if pre != -1:
        ans = ans + pre
    return 1 + abs(ans)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    score = pow(3, player_score + opponent_score)
    last = score % 10
    while score // 10 != 0:
        score = score // 10
    return last == score % 10
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 6
        ...
        while score0 < goal and score1 < goal:
            if who == 0:
                num_rolls = strategy0(score0, score1)  
                current = take_turn(num_rolls, score1, dice)     
                ...
                num_rolls = strategy1(score1, score0)
                current = take_turn(num_rolls, score0, dice)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    maxV = 0
    while i < 11:
        averageFunc = make_averaged(roll_dice, num_samples)
        current = averageFunc(i, dice)
        if current > maxV:
            maxV = current
            ans = i
        i = i + 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(expected_score, opponent_score):
        if opponent_score > expected_score:
            return 0
        else:
            return num_rolls
    else:
