...
def couple(lst1, lst2):
    ...
    length = len(lst2)
    return [[lst1[i], lst2[i]] for i in range(length)]
...
def distance(city1, city2):
    ...
    5.0
    """
    long1, long2 = get_lon(city1), get_lon(city2)
...
def closer_city(lat, lon, city1, city2):
    ...
    return get_name(city1) if distance(city1, city_target) < distance(city2, city_target) else get_name(city2)
...
def nut_finder(t):
    ...
    if label(t) == 'nut':
        ...
        return any([nut_finder(b) for b in branches(t)])
...
def add_trees(t1, t2):
    ...
    if not t1 or not t2:
        return t1 or t2
    return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip_longest(branches(t1), branches(t2))])
