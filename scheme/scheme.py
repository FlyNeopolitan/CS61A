...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        child = Frame(self)
        currentFormal, currentVal = formals, vals
        while currentFormal is not nil:
            name = currentFormal.first 
            value = currentVal.first
            child.define(name, value)
            currentFormal, currentVal = currentFormal.rest, currentVal.rest 
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # python_args = []
        def schemeList_to_pythonList(target):
            if target is nil:
                return []
            first = target.first
            rst = target.rest
            return [first] + schemeList_to_pythonList(rst)
        python_args = schemeList_to_pythonList(args)
        # BEGIN PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # BEGIN PROBLEM 9
        functionBody = expressions.rest
        functionName = target.first
        arguments = target.rest
        #ensure arguments are valid
        validate_formals(arguments)
        #bind function name to function body
        env.define(functionName, LambdaProcedure(arguments, functionBody, env))
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while current.rest is not nil:
        value = scheme_eval(current.first, env)
        if is_false_primitive(value):
            return False
        else:
            current = current.rest
    lastValue = scheme_eval(current.first, env)
    if is_true_primitive(lastValue):
        return lastValue
    return False
    # END PROBLEM 12
