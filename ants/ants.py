...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        currentPlace, distance = self.place, 0
        while type(currentPlace) != Hive:
            currentAnt = random_or_none(currentPlace.bees)
            if currentAnt != None and distance >= self.min_range and distance <= self.max_range:
                return currentAnt
            else:
                currentPlace, distance = currentPlace.entrance, distance + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        Damage = amount if amount < self.armor else amount + self.damage
        currentPlace = self.place
        activeBees = list(currentPlace.bees)
        for i in range(len(currentPlace.bees)):
            activeBees[i].reduce_armor(Damage)
        currentPlace.bees = []
        for i in range(len(activeBees)):
            if activeBees[i].place:
                currentPlace.bees.append(activeBees[i])
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            targetBee = random_or_none(self.place.bees)
            if targetBee:
                self.eat_bee(targetBee)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        activeBees = list(currentPlace.bees)
        for i in range(len(currentPlace.bees)):
            activeBees[i].reduce_armor(self.damage)
        currentPlace.bees = []
        for i in range(len(activeBees)):
            if activeBees[i].place:
                currentPlace.bees.append(activeBees[i])
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        def buff(targetAnt):
            if targetAnt != None and not targetAnt.buffed:
                targetAnt.damage *= 2
                targetAnt.buffed = True

        if not self.real_Queen:
            Insect.reduce_armor(self, self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            currentPlace = self.place.exit
            while currentPlace != None:
                currentAnt = currentPlace.ant
                buff(currentAnt)
                if isinstance(currentAnt, ContainerAnt) and currentAnt.contained_ant is not None:
                    buff(currentAnt.contained_ant)
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if amount >= self.armor and self.real_Queen:
            bees_win()
        Insect.reduce_armor(self, amount)
        # END Problem 13
